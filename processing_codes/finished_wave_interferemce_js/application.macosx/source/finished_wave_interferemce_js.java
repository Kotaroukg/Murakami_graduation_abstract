...
public class finished_wave_interferemce_js extends PApplet {
...
public void make_elementary_waves(){
  for(int i = 0; i < elementary_waves_array.length; i++){
    Elementary_waves elementary_waves = new Elementary_waves(width/2-50,size/2,40.0f,5.0f,1.0f,10.0f,10.0f,100.0f);//\u3068\u308a\u3042\u3048\u305a\u4f5c\u3063\u3066\u3044\u308b.
...
public void draw_wave(){
  for(int i = 0; i < s_p; i++){
    for(int j = 0; j < s_p; j++){
      ...
      if(number_wave_point != 0 || point[i][j] != 0.0f){stroke(point[i][j]*(230/(number_wave_point*2)),100,100,100);
...
//stroke(0,0,0,0);
  for(int i = 0; i < s_p; i++){
    for(int j = 0; j < s_p; j++){
      ...
      if(j==check_y) point(i*point_regulation,(size/2+(point[i][j]*3.0f/(float)number_wave_point)));
...
class Sliders{
  ...
  public float draw_slider(float variable_value,int i){
    if(draw_flag == true){
...
class Elementary_waves{
  ...
  public float now_displacement_y(float distance, float lambda, float period, float time_adjustment){
    ...
    time_phase_difference = ((millis()-time_adjustment)/1000.0f)*(360.0f / period); //\u3053\u3053\u3067\u6642\u9593\u5dee\u3092\u8a08\u7b97\u3057\u3066\u3044\u308b\u2192\u5909\u6570\u3067\u958b\u59cb\u6642\u523b\u3092\u8a18\u9332\u2192\u3053\u308c\u3092\u4ee3\u5165\u3059\u308c\u3070\u3044\u3044\uff0e
    float a = (distance%lambda)/lambda*(360.0f);
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--full-screen", "--bgcolor=#666666", "--stop-color=#cccccc", "finished_wave_interferemce_js" };
    if (passedArgs != null) {
