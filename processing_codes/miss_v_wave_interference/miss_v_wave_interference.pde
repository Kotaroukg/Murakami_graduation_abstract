...
//stroke(0,0,0,0);
  for(int i = 0; i < s_p; i++){
    for(int j = 0; j < s_p; j++){
      ...
      if(j==check_y) point(i*point_regulation,(size/2+(point[i][j]*3.0/(float)number_wave_point)));
      //println(point[i][j]);//number_wave_point*3);
    }
  }
...
class Sliders{
  ...
  float draw_slider(float variable_value,int i){
    if(draw_flag == true){
      int num_separator = (int)map(variable_value, min, max, 0, number_tickmarks-1); //temper=グローバル min~maxの範囲を０からnumber_tickmarksに変換
      ...
      if(slider_dragged){
        ...
        if(digit == 0){
          variable_value = round(map(num_separator, 0, number_tickmarks-1, min, max));
        }else{
          variable_value = map(num_separator, 0, number_tickmarks-1, min, max);
        }
        if(i != -1){
          
          if(lambda_mode == true){
            float a = variable_value/elementary_waves_array[i].record_lambda;//
            elementary_waves_array[i].period = elementary_waves_array[i].period * a;//
            elementary_waves_array[i].record_lambda = variable_value; //ここでvが一定になるように調整

          }else{
            float a = variable_value/elementary_waves_array[i].record_period;//
            elementary_waves_array[i].lambda = elementary_waves_array[i].lambda * a;//
            elementary_waves_array[i].record_period = variable_value; //ここでvが一定になるように調整
          }
        }
...
class Elementary_waves{
  ...
  Elementary_waves(float _center_x, float _center_y, float _lambda, float _period, 
    float _period_max, float _period_min, float _lambda_max, float _lambda_min){
    center_x = _center_x;
    center_y = _center_y;
    lambda = _lambda;
    period = _period;
