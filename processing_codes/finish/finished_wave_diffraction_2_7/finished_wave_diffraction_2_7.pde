...
void draw(){  
  ...
  slider2(size+50,20,size-50,30,5.0,50.0,20);
  strokeWeight(5);
  for(int i = 0; i < number_wave_point; i++){
    point((size/2)-((i-2.5)*point_width),height/2);
  }
...
void keyPressed(){
  if((keyPressed == true) && keyReleased == true &&((key == 's') ||  (key == 'S'))){
    ...
    draw_start = true;
    for(int i = 0; i < number_wave_point; i++){
      elementary_waves_array[i].center_x = size/2-(point_width*(number_wave_point-1)/2.0)+i*point_width;
...
//slider2(size+100,20,size-50,30,5.0,50.0,20);
}
...
void slider2(int position_x, int position_y, int slider_height, int slider_width, float min, float max, int number_tickmarks){
  ...
  int num_separator = (int)map(point_width, min, max, 0, number_tickmarks-1); //temper=グローバル min~maxの範囲を０からnumber_tickmarksに変換
  float cordinate_y = position_y + slider_height - ((float)slider_height / (number_tickmarks-1))*num_separator;
  fill(255);
  noStroke();                                                        //text消すため
  rect(position_x, position_y + slider_height, slider_width+50, 30); 
  ...
  text( "point_width="+nf(point_width,1,0), position_x, position_y + slider_height + 20);
  //text( "slit width =" + point_width * 5 , size + 70, position_y + slider_height/2);
  ...
  if(slider_dragged2){
    ...
    point_width=round(map(num_separator, 0, number_tickmarks-1, min, max));
