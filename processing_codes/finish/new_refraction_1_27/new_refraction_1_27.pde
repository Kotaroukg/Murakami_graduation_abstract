...
void setup(){  
  ...
  void make_waves_instance(){
    for(int i = 0; i < elementary_waves_array.length; i++){
        Elementary_waves elementary_waves = new Elementary_waves(((-cross_line_x)+(float)point_width*(i-2)),size/2,lambda*refractive_index,f_wave.x); //点源５つなのでi-2している
