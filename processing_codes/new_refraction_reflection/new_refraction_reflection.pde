  ...
  float lambda = 40.0;
  float period = 1.5;
  float refractive_index = 1.2;
  float record_lambda;
  float average_slope = 0;
  int max_phase_count = 0;
  ...
  void make_waves_instance(){
    for(int i = 0; i < elementary_waves_array.length; i++){
      if(i == 0){
        Elementary_waves elementary_waves = new Elementary_waves(0,0,lambda,period); 
        elementary_waves_array[i] = elementary_waves;
      }else{
        if(i <= elementary_waves_array.length/2){
          Elementary_waves elementary_waves = new Elementary_waves(100+(float)point_width/((float)maximum_waves_number-2)*(float)(i-1),size/2,lambda*refractive_index,period);
          elementary_waves_array[i] = elementary_waves; 
        }else{
          Elementary_waves elementary_waves = new Elementary_waves(100+(float)point_width/((float)maximum_waves_number-2)*(float)(i-1),size/2,lambda,period);
          elementary_waves_array[i] = elementary_waves;
        }
      }
      int s_p_n = s_p * i;
    ...
    //println(elementary_waves_array[i].lambda);
  }
}
...
void calculate_i_point_to_e_point(){
  ...
  void draw_wave(){ //波が一つのバージョン
      ...
      for(int j = 0; j < s_p; j++){
        ...
        for(int k = 0; k < number_wave_point; k++){
          ...
          if(point_distance[i+s_p*k][j] < range && point_distance[i+s_p*k][j] > range2){ //クラス配列，各点と波ごとの距離と範囲を比較
            ...
            if(k != 0) 
            {
              if(i_p>elementary_waves_array[maximum_waves_number/4].center_x){ //iが素元波の中心から左にいったら
                if(j_p < size/2 && k > maximum_waves_number/2){ // 反射波
                  point[i][j] += 2.0*y; //４で割って屈折波の真ん中の点源を中心とする
                }else{
                  if(j_p > size/2 && k <= maximum_waves_number/2)point[i][j] += 2.0*y; //屈折波
                }
              }
            }else{
              if(j<s_p/2 && i_p < elementary_waves_array[maximum_waves_number/4].center_x) point[i][j] += y;//入射波の話
            }
        ...
        //こっからは各pointに溜まった値の処理
        if(number_wave_point > 0 || point[i][j] != 0.0){
          if(j<s_p/2 && i_p < elementary_waves_array[maximum_waves_number/2].center_x){//入射波領域
            point[i][j] += 1.0;
            stroke(point[i][j]*115.0,100,100);//230/2
          }else if(number_wave_point > 1){ //入射波以外の波が一つ以上生まれた時
            if(j > s_p/2){ //最大位相計算
    ...
    float slope = ((float)max_phase_x*(float)point_regulation-elementary_waves_array[maximum_waves_number/2].center_x)/((float)max_phase_y*(float)point_regulation-(float)size/2.0);
    if(number_wave_point>1 && slope > 0){
      max_phase_count++;
      average_slope += slope;
    }
    ...
    if(refractive_index >= 1.0){ 
      if(max_phase_x > s_p - 3) {
        draw_line_point = false;
        
      }
    }else{
      if(max_phase_y > s_p - 3) {
        draw_line_point = false;
    ...
    if(draw_line_point == false){
      //println(average_slope/max_phase_count);
      line(elementary_waves_array[maximum_waves_number/2].center_x,elementary_waves_array[maximum_waves_number/2].center_y,size,size*average_slope/max_phase_count);//1.5はルート２以上
      noLoop();
    }else{
      //println(average_slope/max_phase_count);
      if(elementary_waves_array[maximum_waves_number/2].center_x < max_phase_x*point_regulation){
