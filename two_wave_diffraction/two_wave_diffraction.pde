...
void make_elementary_waves(){
  for(int i = 0; i < elementary_waves_array.length; i++){
    Elementary_waves elementary_waves = new Elementary_waves(size/2-(point_width*(number_wave_point-1)/2.0)+i*point_width,size/4*3,lambda,4.0);
  ...
  for(int i = 0; i < s_p; i++){
    for(int j = 0; j < s_p; j++){
      for(int k = 0; k < number_wave_point; k++){
        ...
        float range2 = range - elementary_waves_array[k].lambda;
